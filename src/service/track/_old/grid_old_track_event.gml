...
///@static
///@type {Struct}
global.__grid_old_track_event = {
  "brush_grid_old_channel": {
    run: function(data) {
  ...
  "brush_grid_old_particle": {
    run: function(data) {
      ...
      var event = particleService.factoryEventSpawnParticleEmitter({
        ...
        shape: ParticleEmitterShape.get(Struct.get(data, "grid-particle_shape")),
        distribution: ParticleEmitterDistribution.get(Struct.get(data, "grid-particle_distribution")),
      })
      particleService.send(event)
    },
  },
  "brush_grid_old_player": {
    run: function(data) {
      var controller = Beans.get(BeanVisuController)
      var json = {
  ...
  "brush_grid_old_separator": {
    run: function(data) {
      ...
      if (Struct.get(data, "grid-separator_use-mode")) {
        controller.gridService.properties.separatorsMode = Struct.get(data, "grid-separator_mode")
      }
...
#macro grid_old_track_event global.__grid_old_track_event
