...
///@param {Struct} json
///@return {Struct}
function migrateGridOldCoinEvent(json) {
  if (!Optional.is(Struct.getIfType(json, "grid-coin_channels-spawn-x", Number))) {
    var spawnX = (clamp(Struct.getDefault(json, "grid-coin_spawn-x", 0.0), -3.5, 4.5) - 0.5)
    var channel = (spawnX / (SHROOM_SPAWN_CHANNEL_SIZE / SHROOM_SPAWN_CHANNEL_AMOUNT))
    Struct.set(json, "grid-coin_spawn-x", channel)
  }

  if (!Optional.is(Struct.getIfType(json, "grid-coin_channels-spawn-y", Number))) {
    var spawnY = (clamp(Struct.getDefault(json, "grid-coin_spawn-y", 0.0), -4.5, 3.5) + 0.5)
    var row = (spawnY / (SHROOM_SPAWN_ROW_SIZE / SHROOM_SPAWN_ROW_AMOUNT))
    Struct.set(json, "grid-coin_spawn-y", row)
  }
  ...
  return {
    ...
    "en-coin_snap-x": Struct.getIfType(json, "grid-coin_use-snap-h", Boolean, true),
    "en-coin_use-rng-x": Struct.getIfType(json, "grid-coin_use-spawn-x", Boolean, false),
    "en-coin_y": Struct.getIfType(json, "grid-coin_spawn-y", Number, 0.0),
    "en-coin_snap-y": Struct.getIfType(json, "grid-coin_use-snap-v", Boolean, true),
    "en-coin_use-rng-y": Struct.getIfType(json, "grid-coin_use-spawn-y", Boolean, false),
    "en-coin_rng-x": Struct.getIfType(json, "en-coin_rng-x", Number, 0.0),
    "en-coin_rng-y": Struct.getIfType(json, "en-coin_rng-y", Number, 0.0),
  }
}
...
///@param {?Struct} [json]
///@return {Struct}
function brush_grid_old_coin(json = null) {
  return {
